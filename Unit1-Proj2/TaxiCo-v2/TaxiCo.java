...
public class TaxiCo {
    ...
    /**
     * Return the vehicle available to do the traject of client
     * @param   destination  The destination of the client
     * @return  return the vehicle available, if no return null
     */
    public Vehicle vehicleOrder (String destination) {
        for(Vehicle vehicle : vehicleFleet) {
            if(isShuttle(vehicle)) {
                Shuttle s = (Shuttle) vehicle;
                if(s.getDestination().equals(destination)) return vehicle;
            }
        }
        for (Vehicle vehicle : vehicleFleet) {
            if(isTaxi(vehicle)) {
                Taxi t = (Taxi) vehicle;
                if(t.getDestination() == null) return vehicle;
            }
        }
    ...
    /**
     * Determines if a vehicle is a Taxi.
     * @param      amITaxi  The am i taxi
     * @return     True if taxi, False otherwise.
     */
    private boolean isTaxi (Vehicle amITaxi) {
        if (amITaxi instanceof Taxi) return true;
        else return false;
    }

    /**
     * Determines if a vehicle is  a Shuttle.
     * @param      amIShuttle  The am i shuttle
     * @return     True if shuttle, False otherwise.
     */
    private boolean isShuttle (Vehicle amIShuttle) {
        if (amIShuttle instanceof Shuttle) return true;
        else return false;
    }
}